...
export async function GET(request) {
  try {
    ...
    // Safely parse watched_with JSONB field for each row
    const parsedRows = rows.map((row) => {
      let watched_with = [];
      try {
        if (typeof row.watched_with === "string") {
          watched_with = JSON.parse(row.watched_with);
        } else if (Array.isArray(row.watched_with)) {
          watched_with = row.watched_with;
        }
      } catch (e) {
        console.warn(
          "Failed to parse watched_with for row:",
          row.id,
          e?.message,
        );
        watched_with = [];
      }
      return { ...row, watched_with };
    });
    ...
    return Response.json({ history: parsedRows, stats: stats[0] });
  } catch (e) {
...
export async function POST(request) {
  try {
    ...
    // Safely parse watched_with field before returning
    const entry = rows[0];
    let parsed_watched_with = [];
    try {
      if (typeof entry.watched_with === "string") {
        parsed_watched_with = JSON.parse(entry.watched_with);
      } else if (Array.isArray(entry.watched_with)) {
        parsed_watched_with = entry.watched_with;
      }
    } catch (e) {
      console.warn(
        "Failed to parse watched_with in POST response:",
        e?.message,
      );
      parsed_watched_with = [];
    }
    ...
    return Response.json(
      { entry: { ...entry, watched_with: parsed_watched_with } },
      { status: 201 },
    );
  } catch (e) {
