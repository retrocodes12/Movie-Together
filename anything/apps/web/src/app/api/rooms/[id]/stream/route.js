...
async function getRoomSnapshot(roomId) {
  ...
  try {
    if (typeof room.playback_state === "string") {
      playback_state = JSON.parse(room.playback_state);
    } else if (room.playback_state && typeof room.playback_state === "object") {
      ...
      const poll = async () => {
        ...
        try {
          ...
          if (currentHash !== lastSnapshotHash) {
            ...
            try {
              const prev = lastSnapshotHash ? JSON.parse(lastSnapshotHash) : {};
              ...
              // If parsing fails, default to ROOM_UPDATED
              console.warn(
                "SSE event type detection parse error:",
                parseError?.message,
              );
