...
export async function GET(request) {
  try {
    ...
    // Safely parse payload JSONB field for each vote
    const parsedVotes = votes.map((vote) => {
      let payload = {};
      try {
        if (typeof vote.payload === "string") {
          payload = JSON.parse(vote.payload);
        } else if (vote.payload && typeof vote.payload === "object") {
          payload = vote.payload;
        }
      } catch (e) {
        console.warn(
          "Failed to parse vote payload for vote:",
          vote.id,
          e?.message,
        );
        payload = {};
    ...
    // Attach my ballot
    let myBallots = {};
    if (me && parsedVotes.length) {
      const voteIds = parsedVotes.map((v) => v.id);
