...
function safeSetCurrentTime(player, pos) {
  if (!player) return false;
  if (!isFiniteNumber(pos) || pos < 0) return false;
  try {
    player.currentTime = pos;
    return true;
  } catch (e) {
    console.warn("safeSetCurrentTime failed:", e?.message);
    return false;
  }
}

function safeReplace(player, url) {
  if (!player || !isValidUrl(url)) return false;
  try {
    player.replace({ uri: url.trim() });
    return true;
  } catch (e) {
    console.warn("safeReplace failed:", e?.message);
    return false;
  }
}
...
export function useVideoSync({
  ...
  const lastAppliedUrlRef = useRef(null);
  ...
  useEffect(() => {
    ...
    // Content change — only if URL is valid AND has actually changed
    if (isValidUrl(content_url) && content_url !== lastAppliedUrlRef.current) {
      if (safeReplace(player, content_url)) {
        lastAppliedUrlRef.current = content_url;
      }
    }
    ...
    // Position sync — only apply if diff is meaningful AND value is finite
    if (isFiniteNumber(position)) {
      const currentPos = player.currentTime ?? 0;
      const drift = Math.abs(currentPos - position);
      if (drift > SEEK_SYNC_THRESHOLD) {
        safeSetCurrentTime(player, position);
      }
    }
    ...
    if (status !== lastAppliedStatusRef.current) {
      ...
      try {
        ...
        console.warn("useVideoSync: play/pause failed", e?.message);
      }
    }
  ...
  useEffect(() => {
    ...
    const check = () => {
      ...
      if (!isFiniteNumber(expectedPos)) return;
      const drift = Math.abs(localPos - expectedPos);
      if (drift > SEEK_SYNC_THRESHOLD) {
        safeSetCurrentTime(player, expectedPos);
      }
    };
  ...
  const handlePlay = useCallback(() => {
    ...
    onPlay?.(isFiniteNumber(pos) ? pos : 0);
  ...
  const handlePause = useCallback(() => {
    ...
    const pos = player.currentTime ?? 0;
    onPause?.(isFiniteNumber(pos) ? pos : 0);
  }, [player, acquireLock, onPause]);

  const handleSeek = useCallback(
    (position) => {
      if (!player) return;
      if (!isFiniteNumber(position) || position < 0) return;
      acquireLock();
      safeSetCurrentTime(player, position);
  ...
  const handleSkipForward = useCallback(
    (seconds = 10) => {
      ...
      const newPos = Math.min(current + seconds, dur);
      if (isFiniteNumber(newPos)) {
        safeSetCurrentTime(player, newPos);
      }
