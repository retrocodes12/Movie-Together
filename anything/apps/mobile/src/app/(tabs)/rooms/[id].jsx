...
export default function WatchRoomScreen() {
  ...
  const hasPlaybackControl = sync.hasPlaybackControl;
  ...
  const handleLeave = () => {
    Alert.alert(
      ...
      isHost
        ? "You'll leave as host. The room will continue and another member will become host."
  ...
  // Grant/revoke playback control for a member (host only, private rooms)
  const handleToggleControl = async (targetUserId, currentlyHasControl) => {
    try {
      const action = currentlyHasControl ? "revoke" : "grant";
      const res = await fetch(`/api/rooms/${id}/permissions`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          device_id: deviceId,
      ...
      Alert.alert("Error", "Could not update playback controls.");
    }
  };
            ...
            return (
              ...
              >
                ...
                <View style={{ flexDirection: "row", gap: 4 }}>
                  ...
                  {/* Co-host badge — visible to everyone */}
                  {room.host_id !== member.user_id &&
                    member.has_playback_control && (
                      <View
                        style={{
                          flexDirection: "row",
                          alignItems: "center",
                  ...
                  {/* Grant/Revoke Controls button — host only, private room, not for themselves */}
                  {isHost && isPrivate && room.host_id !== member.user_id && (
                    <TouchableOpacity
                      onPress={() =>
                        handleToggleControl(
                          member.user_id,
                      ...
                      style={{
                        backgroundColor: member.has_playback_control
                          ? "rgba(239,68,68,0.12)"
                          : "rgba(34,197,94,0.12)",
                        borderRadius: Radius.full,
                        paddingHorizontal: 9,
                        paddingVertical: 3,
                        borderWidth: 1,
                        borderColor: member.has_playback_control
                          ? "rgba(239,68,68,0.3)"
                          : "rgba(34,197,94,0.3)",
                      }}
                      activeOpacity={0.7}
                    >
                      <Text
                        style={{
                          fontSize: 10,
                          color: member.has_playback_control
                            ? "#EF4444"
                            : "#22C55E",
                          fontFamily: "Inter_600SemiBold",
                        }}
                      >
                        {member.has_playback_control ? "Revoke" : "Controls"}
                      </Text>
                    </TouchableOpacity>
                  )}
                </View>
