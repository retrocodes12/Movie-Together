...
function addonParam(addons) {
  return encodeURIComponent(
    JSON.stringify(addons.filter((a) => a.enabled).map((a) => a.url))
  );
}
...
function yearOf(item) {
  return (
    item?.year ||
    item?.releaseInfo ||
    (item?.released ? new Date(item.released).getFullYear() : "")
  );
}
...
function pickHomeCatalogs(catalogs, type) {
  ...
  for (const catalog of typed.filter((c) => c.showInHome !== false)) {
    ...
    if (
      !picked.some(
        (p) =>
          p.catalog.id === catalog.id &&
          p.catalog.addon.url === catalog.addon.url
      )
    ) {
...
function AddonBadge({ label, C }) {
  return (
    ...
    >
      <Text
        style={{ ...Typography.meta, color: C.foregroundMuted, fontSize: 10 }}
      >
        {label}
      </Text>
    </View>
...
export default function ContentSearch({ visible, onClose, onSelectStream }) {
    ...
    async (source = addons) => {
      try {
        const data = await api(
          `/api/stremio?action=addons&addons=${addonParam(source)}`
        );
  ...
  const loadCatalogs = useCallback(async () => {
    try {
      const data = await api(
        `/api/stremio?action=catalogs&addons=${enabledAddonParam}`
      );
  ...
  const doSearch = useCallback(
    async (q, type) => {
      ...
      try {
        const data = await api(
          `/api/stremio?query=${encodeURIComponent(
            q
          )}&type=${type}&addons=${enabledAddonParam}`
        );
  ...
  const openCatalog = async (catalog, genre = "") => {
    ...
    try {
      const data = await api(
        `/api/stremio?action=catalog&type=${
          catalog.type
        }&catalog_id=${encodeURIComponent(catalog.id)}` +
          `&addon_url=${encodeURIComponent(
            catalog.addon.url
  ...
  const loadStreamsFor = useCallback(
    async (item, video = null) => {
      ...
      try {
        const data = await api(
          `/api/stremio?action=streams&type=${
            item.type || contentType
          }&video_id=${encodeURIComponent(id)}` +
  ...
  const handleSelectContent = async (item) => {
    ...
    try {
      const data = await api(
        `/api/stremio?action=meta&type=${
          item.type || contentType
        }&id=${encodeURIComponent(item.id)}` +
          `${
            item.addon?.url
              ? `&addon_url=${encodeURIComponent(item.addon.url)}`
              : ""
          }&addons=${enabledAddonParam}`
      );
  ...
  const handleDirectUrl = async () => {
    ...
    try {
      const data = await api(
        `/api/stremio?validate_url=${encodeURIComponent(url)}`
      );
      if (!data.valid) {
        Alert.alert(
          "URL not reachable",
          "The server could not validate that URL. It may still fail in the player."
        );
      }
    } catch {}
  ...
  const addAddon = async () => {
    ...
    try {
      const data = await api(
        `/api/stremio?action=manifest&addon_url=${encodeURIComponent(url)}`
      );
      const next = [
  ...
  const toggleAddon = async (url) => {
    await saveAddons(
      addons.map((a) => (a.url === url ? { ...a, enabled: !a.enabled } : a))
    );
  };
      ...
      {selectedCatalog ? (
              ...
              {["", ...selectedCatalog.genres].slice(0, 24).map((genre) => (
                <TouchableOpacity
                  ...
                  style={{
                    ...
                    paddingVertical: 6,
                    backgroundColor:
                      catalogGenre === genre ? C.primary : C.canvasMuted,
                  }}
                >
                  <Text
                    style={{
        ...
        <>
          ...
          {sections.map(({ key, Icon, catalog, items }) => (
            <View
              key={`${catalog.addon.url}:${catalog.type}:${catalog.id}`}
      ...
      <View style={{ marginHorizontal: Spacing.xl, marginBottom: Spacing.lg }}>
        ...
        >
          ...
          >
            <Play
              size={18}
              color={directUrl.trim() ? C.primary : C.foregroundMuted}
            />
          </TouchableOpacity>
        </View>
      </View>
  ...
  const renderAddons = () => (
    ...
    >
      ...
      >
        ...
        <TouchableOpacity
          onPress={addAddon}
          disabled={!newAddonUrl.trim() || loading}
        >
          <Text
            style={{
              color: C.primary,
          ...
          >
            ...
            <View style={{ flex: 1 }}>
              ...
              >
                ...
                <AddonBadge
                  label={`${addon.resources?.length || 0} resources`}
                  C={C}
                />
                <AddonBadge
                  label={`${addon.catalogs?.length || 0} catalogs`}
                  C={C}
                />
              </View>
            </View>
      ...
      <View style={{ flex: 1 }}>
        ...
        >
          ...
          >
            {meta.poster ? (
              <Image
                ...
                style={{ width: 82, height: 122, borderRadius: Radius.sm }}
                contentFit="cover"
              />
            ) : (
              <View
            ...
            <View style={{ flex: 1 }}>
              ...
              >
                {!!yearOf(meta) && (
                  <AddonBadge label={String(yearOf(meta))} C={C} />
                )}
                {!!meta.imdb_rating && (
                  <AddonBadge label={`IMDb ${meta.imdb_rating}`} C={C} />
                )}
          ...
          {streams.map((stream, idx) => (
            <TouchableOpacity
              key={`${stream.source}:${idx}:${
                stream.url || stream.infoHash || stream.externalUrl
              }`}
            ...
            >
              <View
                style={{
                  ...
                  height: 36,
                  borderRadius: Radius.sm,
                  backgroundColor: stream.playable
                    ? C.primarySoft
                    : C.canvasMuted,
                  alignItems: "center",
                  justifyContent: "center",
              ...
              >
                <Play
                  size={16}
                  color={stream.playable ? C.primary : C.foregroundMuted}
                />
              </View>
              <View style={{ flex: 1 }}>
                <Text
                ...
                <Text
                  ...
                  numberOfLines={2}
                >
                  {stream.source} · {stream.quality} · {stream.streamType} ·{" "}
                  {stream.description || stream.title}
                </Text>
              </View>
              <AddonBadge
                label={stream.playable ? "Play" : stream.streamType}
                C={C}
              />
            </TouchableOpacity>
          ))}
