...
export default function VideoPlayer({
  ...
  const player = useVideoPlayer(validatedSource, (p) => {
    try {
      p.playbackRate = playbackState?.speed || 1.0;
      p.allowsExternalPlayback = true;
    } catch (e) {
      console.warn("Player init error:", e?.message);
    }
  });

  const lastReplacedUrlRef = useRef(validatedSource?.uri || null);
  ...
  // ── Global error suppression for expo-video web quirks ────────────────────
  useEffect(() => {
    ...
    const handler = (event) => {
      ...
      if (
        ...
        // Swallow expo-video web quirks — they don't break playback
        event.preventDefault?.();
        console.warn(
          "Suppressed NotSupportedError from video player:",
          err.message,
        );
      }
    };
  ...
  useEffect(() => {
    if (status === "error") {
      ...
      if (retryCount < 3 && validatedSource?.uri) {
        const delay = Math.pow(2, retryCount) * 1500;
        const timer = setTimeout(() => {
          try {
            player.replace(validatedSource);
            lastReplacedUrlRef.current = validatedSource.uri;
            setRetryCount((c) => c + 1);
          } catch (e) {
            console.warn("Retry replace failed:", e?.message);
          }
        }, delay);
  ...
  const handleSubtitleChange = useCallback(async (url) => {
    ...
    try {
      ...
      console.warn("Subtitle parse failed:", e?.message);
      setSubtitleCues([]);
    }
  }, []);
  ...
  // Safe seek helper — guards against NaN/Infinity which crash expo-video web
  ...
  const handleProgressResponderGrant = (e) => {
    ...
    if (Number.isFinite(pos)) setSeekPosition(pos);
  };
  ...
  }, [player, validatedSource]);

  // ── Content URL change ────────────────────────────────────────────────────

  useEffect(() => {
    if (!validatedSource?.uri) return;
    // Skip if URL hasn't changed — prevents redundant replace() calls that throw on web
    if (lastReplacedUrlRef.current === validatedSource.uri) return;
    try {
      player.replace(validatedSource);
      lastReplacedUrlRef.current = validatedSource.uri;
    } catch (e) {
      console.warn("Content replace failed:", e?.message);
    }
  }, [validatedSource?.uri]);
