...
import { WatchTogetherSync } from "../../../core/watchTogether/watchTogetherSync.js";
...
export const PlayerScreen = {
  ...
  async mount(params = {}) {
    ...
    this.watchTogetherRoomId = params.watchTogetherRoomId || null;
    this.watchTogetherRoom = null;
    this.watchTogetherPanelVisible = Boolean(this.watchTogetherRoomId);
    ...
    if (this.watchTogetherRoomId && !this.isExternalFrameMode()) {
      WatchTogetherSync.attach({
        roomId: this.watchTogetherRoomId,
        PlayerController,
        canControl: true,
        onUpdate: (room) => {
          this.watchTogetherRoom = room;
          this.renderWatchTogetherPanel();
        }
      });
    }
  ...
  renderPlayerUi() {
    ...
    if (this.isExternalFrameMode()) {
      ...
      root.innerHTML = `
        ...
        <div id="playerWatchTogetherPanel" class="player-watch-together-panel hidden"></div>
  ...
  getControlDefinitions() {
    ...
    if (this.watchTogetherRoomId) {
      base.push({ action: "watchTogether", label: "WT", title: "Watch Together" });
    }
  ...
  renderWatchTogetherPanel() {
    const panel = this.container?.querySelector("#playerWatchTogetherPanel");
    if (!panel) return;
    if (!this.watchTogetherRoomId || !this.watchTogetherPanelVisible) {
      panel.classList.add("hidden");
      panel.innerHTML = "";
      return;
    }
    const room = this.watchTogetherRoom || {};
    const inviteCode = room.invite_code || this.params.watchTogetherInviteCode || "";
    const inviteUrl = this.params.watchTogetherInviteUrl || WatchTogetherClient.inviteUrl(room);
    const members = Array.isArray(room.members) ? room.members : [];
    panel.classList.remove("hidden");
    panel.innerHTML = `
      <div class="watch-together-head">
        <div>
          <div class="watch-together-title">Watch Together</div>
          <div class="watch-together-subtitle">${escapeHtml(members.length ? `${members.length} in room` : "Room sync active")}</div>
        </div>
        <button class="watch-together-close" data-action="watchTogetherClose">×</button>
      </div>
      <div class="watch-together-code">${escapeHtml(inviteCode || "------")}</div>
      <button class="watch-together-wide" data-action="watchTogetherCopy" data-invite="${escapeHtml(inviteUrl)}">Copy invite</button>
      <div class="watch-together-actions">
        <button data-action="watchTogetherDiscussion">Discuss</button>
        <button data-action="watchTogetherResume">Resume</button>
        <button data-action="watchTogetherMute">Mute</button>
      </div>
      <div class="watch-together-reactions">
        ${["😂", "😱", "🔥", "❤️", "😭", "👏"].map((emoji) => `<button data-action="watchTogetherReact" data-emoji="${emoji}">${emoji}</button>`).join("")}
      </div>
      <div class="watch-together-members">
        ${members.map((member) => `
          <div class="watch-together-member">
            <span>${escapeHtml(member.display_name || member.username || "Guest")}</span>
            <span>${member.can_control ? "control" : "watch"}</span>
          </div>
        `).join("")}
      </div>
    `;
    panel.querySelectorAll("[data-action]").forEach((node) => {
      node.addEventListener("click", () => {
        ...
        if (action === "watchTogetherReact") {
          WatchTogetherSync.react(node.dataset.emoji || "").catch(() => {});
          return;
        }
  ...
  performControlAction(action) {
    ...
    if (action === "watchTogetherClose") {
      this.watchTogetherPanelVisible = false;
      this.renderWatchTogetherPanel();
      return;
    }

    if (action === "watchTogetherCopy") {
      const invite = this.params.watchTogetherInviteUrl || WatchTogetherClient.inviteUrl(this.watchTogetherRoom || {});
      navigator.clipboard?.writeText?.(invite).catch(() => {});
      return;
    }

    if (action === "watchTogetherDiscussion") {
      WatchTogetherSync.startDiscussion(10).catch(() => {});
      return;
    }

    if (action === "watchTogetherResume") {
      WatchTogetherSync.endDiscussion().catch(() => {});
      WatchTogetherSync.command("PLAY").catch(() => {});
      return;
    }

    if (action === "watchTogetherMute") {
      const nextMuted = !WatchTogetherSync.voiceState.muted;
      WatchTogetherSync.setVoiceState({ muted: nextMuted, speaking: false }).catch(() => {});
      return;
    }
